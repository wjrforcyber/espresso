...
{
    pset_family Abar;
    register pset p, p1, restrict_var;
    register int i;
	 ...
	 */
	restrict_var = set_new(A->sf_size);
	min_set_ord = A->sf_size + 1;
	foreachi_set(A, i, p) {
	    if (SIZE(p) < min_set_ord) {
		set_copy(restrict_var, p);
		min_set_ord = SIZE(p);
	    } else if (SIZE(p) == min_set_ord) {
		set_or(restrict_var, restrict_var, p);
	    }
	}
	...
	/* Check for "essential" columns */
	    ...
	    Abar = unate_complement(abs_covered_many(A, restrict_var));
	    sf_free(A);
	    foreachi_set(Abar, i, p) {
		set_or(p, p, restrict_var);
	    }

	/* else, recur as usual */
	} else {
	    max_i = abs_select_restricted(A, restrict_var);
 ...
 *  also belongs to the set "restrict_var"; weight each column of a set as
 *  1 / (set_ord(p) - 1).
 */
static int
abs_select_restricted(A, restrict_var)
pset_family A;
pset restrict_var;
{
    ...
    count = sf_count_restricted(A, restrict_var);
